...
Download destination.
  ...
  to the folder containing this script.
...
Copy downloaded books to Kindle.
  ...
  Without a folder, Windows Kindle
  detection is attempted automatically.
...
function parseArgs(argv) {
  ...
  if (!SOURCES.has(options.source)) {
    throw new Error(
      `Unsupported source "${options.source}". ` +
      `Use alice, url, or manifest.`
    );
  }
  ...
  if (
    ...
    throw new Error(
      '--source manifest requires --manifest'
    );
  }

  if (
    options.url &&
    !isHttpUrl(options.url)
  ) {
    throw new Error(
...
async function getAliceBooks() {
  writeStep(
    'Reading AliceAndBooks catalogue...'
  );
  ...
  if (!response.ok) {
    throw new Error(
      `AliceAndBooks returned HTTP ${response.status} `
    );
  }
...
async function getAliceBookDownload(book) {
  ...
  if (!response.ok) {
    throw new Error(
      `Book page returned HTTP ${response.status} `
    );
  }
...
async function buildAliceDownloadList(
  options
) {
  const books =
    await getAliceBooks();
  ...
  if (!books.length) {
    throw new Error(
      'No books were found on AliceAndBooks.'
    );
  }
  ...
  for (const book of selected) {
    try {
      const download =
        await getAliceBookDownload(book);
...
function normalizeManifestBook(
  book,
  index
) {
  if (!book || typeof book !== 'object') {
    throw new Error(
      `Manifest entry ${index + 1} is invalid.`
    );
  }
...
async function readManifest(
  ...
  try {
    ...
    throw new Error(
      `Invalid JSON manifest: ${error.message} `
    );
  }
  ...
  return books.map(
    normalizeManifestBook
  );
}

async function buildManifestDownloadList(
  options
) {
  const books =
    await readManifest(
  ...
  if (!books.length) {
    throw new Error(
      'The manifest contains no books.'
    );
  }
...
async function downloadFile(
  ...
  for (
    ...
    try {
      ...
      writeStep(
        `Downloading: ${path.basename(destination)} `
      );
  ...
  try {
    ...
    if (!stats.isDirectory()) {
      return {
        ok: false,
        message:
          'Kindle path is not a directory.'
      };
    }
...
async function dryRun(
  ...
  try {
    ...
    writeSuccess(
      `Output parent is writable: ${parent}`
    );


  } catch {
    writeWarn(
      `Output parent does not currently exist or is not writable: ${path.dirname(options.output)} `
    );
  ...
  writeStep(
    'Testing selected source...'
  );

  if (
    options.source === 'alice'
  ) {
    const aliceStatus =
      await checkUrl(
        ALICE_URL,
    ...
    if (aliceStatus.ok) {
      ...
      try {
        ...
        if (!books.length) {
          ...
          writeSuccess(
            `Found ${books.length} book page link(s).`
          );

          writeStep(
            `Testing up to ${sample.length} book page(s)...`
          );

          for (
            const book of sample
          ) {
            try {
              const download =
                await getAliceBookDownload(
                  book
                );
        ...
        writeWarn(
          `Alice catalogue test failed: ${error.message}`
        );
      }
    } else {
      writeWarn(
  ...
  if (
    ...
    if (status.ok) {
      writeSuccess(
        `URL reachable: HTTP ${status.status}`
      );
      ...
      writeWarn(
        `URL test failed: ${status.message}`
      );
    }
  ...
  if (
    ...
    try {
      ...
      writeSuccess(
        `Manifest is valid: ${books.length} book(s)`
      );
      ...
      for (
        ...
        if (status.ok) {
          writeSuccess(
            `${book.title} -> HTTP ${status.status}`
          );
        } else {
          writeWarn(
            `${book.title}: ${status.message}`
          );
        }
      }
    } catch (error) {
      writeWarn(
        `Manifest test failed: ${error.message}`
      );
    }
  ...
  if (options.kindle) {
    writeStep(
      'Testing Kindle connection...'
    );


    const kindlePath =
      getKindlePath(options);

    if (!kindlePath) {
      writeWarn(
        'No Kindle was detected automatically.'
      );
      ...
      if (result.ok) {
        ...
        writeWarn(
          `Kindle test failed: ${result.message}`
        );
      }
    }
...
async function buildDownloadList(
  ...
  switch (options.source) {
    case 'alice':
      return buildAliceDownloadList(
        options
      );


    case 'url':
      return buildUrlDownloadList(
        options
      );

    case 'manifest':
      return buildManifestDownloadList(
        options
      );
...
async function main() {
  ...
  console.log(
    ' Portable Legal Book Downloader'
  );
  console.log(
    '============================================================'
  );
  ...
  writeSuccess(
    `Inventory folder: ${INVENTORY_DIR} `
  );
  ...
  if (options.kindle) {
    ...
    if (!kindlePath) {
      ...
      console.log(
        'Continuing without Kindle copying.'
      );
    } else {
      const kindleTest =
        await testKindle(
      ...
      if (!kindleTest.ok) {
        writeWarn(
          `Kindle is unavailable: ${kindleTest.message}`
        );
  ...
  writeStep(
    `Building download list from ${options.source}...`
  );

  const downloads =
    await buildDownloadList(
      options
    );

  if (!downloads.length) {
    writeWarn(
      'No downloadable books were found.'
    );
  ...
  writeSuccess(
    `Found ${downloads.length} book(s) to process.`
  );
  ...
  for (
    ...
    console.log('');
    writeStep(
      `[${index + 1}/${downloads.length}] ${book.title}`
    );
    ...
    try {
      ...
      if (kindlePath) {
        try {
          ...
          writeSuccess(
            `Copied to Kindle: ${kindleDestination}`
          );
        } catch (error) {
          writeWarn(
            `Kindle copy failed: ${error.message}`
          );
        }
      }
